...
#        self.update_op = self.update_model(self.batch_obs, self.action_input, self.target_q_val)
...
#        b = tf.contrib.layers.fully_connected(a, 4, activation_fn=tf.nn.relu)
    ...
    def select_action(self, obs, evaluation_mode=False):
        ...
        self.vals = self.sess.run(self.q_values,feed_dict={self.observation_input:np.matrix(obs)})
        f = open("q_vals.txt","a")
...
##        gvs = optimizer.compute_gradients(loss_val)
...
#        full_qvals = self.sess.run(self.q_values, feed_dict={self.observation_input:np.matrix(full_obs)})
    ...
    def train(self):
        ...
        while not done:
            ...
            self.qvals.append(0.99*self.vals + 0.01 * (reward + self.gamma * np.max(tempObj)))
            obs  = next_obs
